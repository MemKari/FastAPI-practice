...
@router.post('/files')
async def add_file(file: Annotated[bytes, File(description='to set metadata')]):  # File загружает только содержимое файла,
    return {'File was processed. File size:': len(file)}


@router.post('/upload_file')
async def add_file_with_metadate(upload_file: UploadFile):  # UploadFile загружает метаинформацию вместе с самим содержимым
